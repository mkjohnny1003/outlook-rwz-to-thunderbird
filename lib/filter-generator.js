(function () {
   ...
   * @returns {string}
   */
  static escapeConditionValue(val) {
    ...
    if (typeof val === 'string') {
      ...
      if (Array.isArray(val)) {
        return val.map(v => FilterGenerator.escapeConditionValue(v)).join(' ');
      }
      str = val.email || val.smtp || val.address || val.name || val.value || JSON.stringify(val);
    } else {
      str = String(val);
    }
  ...
  static generateMsgFilterRulesDat(rules, folderMap = new Map(), options = {}) {
    ...
    for (const rule of rules) {
      lines.push(`name="${String(rule.name || '未命名規則').replace(/"/g, '\\"')}"`);
        ...
        for (const act of rule.actions) {
          switch (act.type) {
            case 'move_to_folder': {
              ...
              const fName = String(act.folderName || 'Folder');
              const uri = (folderInfo && folderInfo.uri) || options.defaultFolderUri || `mailbox://nobody@Local%20Folders/${encodeURIComponent(fName)}`;
...
if (typeof globalThis !== 'undefined') {
  globalThis.FilterGenerator = FilterGenerator;
}
if (typeof window !== 'undefined') {
  window.FilterGenerator = FilterGenerator;
}
